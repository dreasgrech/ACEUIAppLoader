...
A mod is one folder and an empty marker file. No registration step, no manifest to edit, nothing to rebuild. `python tools/new_mod.py <name> --title "Nice Name"` writes the folder, a preview page, a harness and a test file that runs the shared kit.
...
```json
{ "version": "0.4.0", "styles": ["mymod.css"], "scripts": ["mymod.js"] }
```

The mod's name is its folder's, and its marker's. Everything else is optional:

| key | |
|---|---|
| `title` | the log prefix; defaults to the name |
| `pages` | which pages to load on; defaults to `["hud.html"]`, `"*"` means every page |
| `files` | other plain file names the mod fetches itself, a `.wasm` say — copied by the install tool, never injected |
...
Before your scripts run, the loader creates the mod's root — `<div id="<name>" data-mod="<name>">` — inside the HUD's positioning container. `ACEUIModLoader.mod("<name>")` then tells the mod who it is, so its script declares none of it:
...
| `mount(attach, detach)` | calls `attach(#<name>)` once the DOM has the root, once per root, and logs the mod's script-loaded line. Passing `detach` is what lets the app drawer really stop the mod |
| `show(on)`, `shown()` | switch this app on or off exactly as the drawer's own switch does — root hidden, mod stopped through its `detach`, started again on the way back |
...
**The hotkey belongs to the loader, not the mod.** `me.toggle()` hands it over, because a mod that is switched off is not running to hold anything — which is exactly when the key is needed.

`me.panel` owns two more that used to be every mod's to remember: the panel must be ticked every frame until its stored position settles (miss it and the widget never returns to where the player left it), and the frame loop must be stopped in `detach` (miss it and a mod switched off in the drawer keeps running for ever).

## Do not write these twice

Five things every panel in this project needed, and that two or three mods had each grown their own copy of before they moved into the library. If you are about to write one of them, it is already here:
...
The last one matters most: a declared setting is stored in both stores, drawn in the mod's own window, reachable from the app drawer, and announced to the mod when it changes. A hand-built toggle is markup, a class, a click handler and a storage key that you then have to keep in step — which is what PedalGraph's attract mode was until it became four lines of `define`.

## Naming your own work

A profiler can see what the page schedules — animation frames, timers, events — because those go through globals it can replace. It cannot see inside your frame callback: those are closures in your IIFE, so the best it can say on its own is "this mod cost 2 ms". Name the parts worth separating and it can say where the 2 ms went:

```js
ACEUIModLoader.section("render", function () { renderFrame(state, v, frac); });
```

Sections nest, so a section inside a section is a child in the tree. With no profiler attached this is a property read and a call through, which is all a mod pays for being profilable when nobody is profiling it.

## Installing it

```
python tools/install_mod.py <repo>/<name>
```

copies the folder into `Saved Games\ACE\mods\uiresources\ACEUIModLoaderMods\` and writes the empty marker. Escape and resume in the car reloads the HUD and picks up changes.

`ACEPedalGraph` is the reference mod; `apps/devconsole` and `apps/profiler` are the same shape one level in.
