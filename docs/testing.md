...
| `tests/test_lib_browser.py`, `tests/lib/reload-session.html` and `reload-disk.html` | the library loading on a fresh page, as it does after Escape and resume (localStorage full, no HUD store yet) and after a game restart (localStorage empty, the HUD store arriving later): the windows remembered as open come back for their owners, a frame later, on the page they belong to. Module state is per page, which is why these are pages of their own |
